...
void mouseTurnleft(void)
{
  ...
  while ((__GmRight.uiPulseCtr+150 ) <= __GmRight.uiPulse);
   ...
   __GmRight.uiPulseCtr=0;
   __GmSPID.sRef=28;  //直线PID的结构体，速度设定值30 to 35
   __GmWPID.sRef=15;  //旋转PID
   GW=0;
   ...
   while(1)
   {
       if(GW>68200)
       {
         break;   //依据状态TURNLEFT触发中断函数
       }                      
   }
   __mazeInfDebug();//数码管显示状态
   __GmWPID.sRef=0;//更新状态
   __GucMouseState   = __GOAHEAD;//更新状态
   GuiSpeedCtr=3;//
...
void __UART0Init (void)
{
    ...
    UARTConfigSet(UART0_BASE, 9600, (UART_CONFIG_WLEN_8 |  //配置串口0，8位数据，1位起始位，1位停止位，用户波特率
